...
/// A reference to a local media file. `path` is a LOCAL ABSOLUTE PATH and this app
/// never copies the bytes — the file stays where the user put it.
///
/// Consequence the provider layer must handle: a hosted broker cannot fetch a local
/// path, so any provider that is not running on this machine needs an upload leg.
...
/// The JSON blob stored in `drafts.body`.
///
...
/// [`Self::normalize`] re-establishes the invariant on every decode AND every
/// encode, so it cannot drift.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DraftBody {
    /// Bumped when the shape changes. A body with NO version is a v1 body and is
    /// migrated on read; see [`Self::decode`].
    ...
    /// the `post_targets` rows written at schedule time.
    #[serde(default)]
    pub account_ids: Vec<String>,
    /// Always length >= 1 after [`Self::normalize`].
    #[serde(default)]
...
pub struct TemplateBody {
    ...
    #[serde(default)]
    pub text: String,
    /// Per-platform starting text, keyed by [`Platform::as_str`]. Omitted entirely
    /// when empty rather than serialized as `{}`, so a template that has never had
    /// a per-platform default is byte-distinguishable from one whose defaults were
    /// cleared.
...
pub fn validate_for_platform(platform: &str, text: &str, media: &[MediaRef]) -> Option<String> {
    ...
    for item in media {
        ...
        if !allowed {
            let shown = if mime.is_empty() { "unknown type" } else { mime };
...
mod tests {
    ...
    fn limits_table_covers_every_platform_and_falls_back_for_unknowns() {
        ...
        assert_eq!(limits_for(Platform::Bluesky).allowed_mime_prefixes, IMAGE_ONLY);
