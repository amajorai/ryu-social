...
/// The provider operation the registry is resolving. Keeping this typed avoids
/// every caller inventing a provider-specific capability string.
...
impl ProviderOperation {
    pub fn is_supported(self, capabilities: PlatformCapabilities) -> bool {
        match self {
            Self::Connect => true,
            Self::Publish => capabilities.publish,
            Self::Engagement => capabilities.read_engagement,
            Self::Inbox => capabilities.read_comments || capabilities.read_dms,
            Self::Reply => capabilities.send_dm || capabilities.read_comments,
        }
    }
}
...
pub trait PlatformProvider: Send + Sync {
    ...
    async fn reply_to_inbox_item(&self, _item: &ProviderInboxItem, _text: &str) -> PublishResult {
...
impl PlatformProvider for UnconfiguredProvider {
    ...
    async fn connect(&self, account: &ProviderAccount) -> anyhow::Result<Option<String>> {
        anyhow::bail!(
            "no provider is configured for {} — ask Ryu to configure a managed provider or add platform credentials first",
            account.platform
        )
    }
