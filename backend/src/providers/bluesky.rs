...
use crate::state::{approved_local_media_path, MAX_MEDIA_BYTES};
        ...
        if media.is_remote() {
            let response = ryu_egress::guarded_request(
                ...
                ryu_egress::GuardedFetchPolicy {
                    allow_http: false,
                    max_body_bytes: MAX_MEDIA_BYTES,
                    max_redirect_hops: 0,
                    timeout: std::time::Duration::from_millis(
                        crate::state::PROVIDER_CALL_TIMEOUT_MS,
                    ),
                },
            )
            .await
            .map_err(|error| anyhow::anyhow!("could not fetch media: {error}"))?;
            if !(200..300).contains(&response.status) {
                anyhow::bail!("could not fetch media: HTTP {}", response.status);
            }
            return Ok(response.body);
        }
        let path = approved_local_media_path(&media.url)?;
        let metadata = tokio::fs::metadata(&path).await?;
        if metadata.len() > MAX_MEDIA_BYTES {
            anyhow::bail!("media exceeds the {} byte upload limit", MAX_MEDIA_BYTES);
        }
        let file = tokio::fs::File::open(path).await?;
        let mut bytes = Vec::with_capacity(metadata.len() as usize);
        let mut bounded = file.take(MAX_MEDIA_BYTES + 1);
...
mod tests {
    ...
    #[tokio::test]
    async fn remote_media_rejects_private_destinations_before_connecting() {
        let provider = BlueskyProvider::new(
            reqwest::Client::new(),
            BlueskyCredentials::new("me", "pw"),
            Some("http://127.0.0.1:1".into()),
        ...
        let error = provider
            .media_bytes(&PublishMedia {
                url: "https://127.0.0.1/media.png".into(),
                mime_type: "image/png".into(),
                alt_text: None,
            })
            .await
            .expect_err("private media destinations must be screened");
        assert!(error.to_string().contains("could not fetch media"));
    }
}
