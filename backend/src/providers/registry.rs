...
impl ProviderCredentials {
    pub fn from_env() -> Self {
        Self {
            composio_api_key: env("RYU_SOCIAL_COMPOSIO_API_KEY").or_else(|| env("COMPOSIO_API_KEY")),
    ...
    pub fn from_node_settings(node: &crate::settings::NodeSettings) -> Self {
        ...
        if bluesky.is_none() {
            let handle_set = node.bluesky_handle.is_some() || env("RYU_SOCIAL_BLUESKY_HANDLE").is_some();
    ...
    for entry in raw.split(',') {
        ...
        match entry.split_once('=') {
            Some((account, provider)) => match ProviderId::parse(provider) {
                ...
                None => tracing::warn!(entry, "ryu-social: ignoring unknown provider in RYU_SOCIAL_ACCOUNT_PROVIDERS"),
            },
            None => tracing::warn!(entry, "ryu-social: ignoring malformed RYU_SOCIAL_ACCOUNT_PROVIDERS entry"),
        }
    }
    pins
}
    ...
    fn by_id(&self, id: ProviderId) -> Option<Arc<dyn PlatformProvider>> {
        match id {
            ...
            ProviderId::Bluesky => self
                ...
                .clone()
                .map(|p| p as Arc<dyn PlatformProvider>),
            ProviderId::Fake => self.inner.fake.clone().map(|p| p as Arc<dyn PlatformProvider>),
...
mod tests {
    ...
    fn a_direct_adapter_beats_the_broker_for_its_own_platform_only() {
        ...
        assert_eq!(registry.provider_for(Platform::X).id(), ProviderId::Composio);
    }
