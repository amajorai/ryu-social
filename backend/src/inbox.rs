...
use crate::models::{
    new_id, now_ms, InboxItem, InboxKind, PlatformCapabilities, SocialAccount, ID_INBOX,
};
use crate::providers::{ProviderAccount, ProviderInboxItem, ProviderOperation, PublishResult};
use crate::state::{AppState, EVENT_INBOX_RECEIVED};
use crate::store::SocialStore;

/// Hard cap on how much of an item's text rides in a notification body.
const NOTIFY_BODY_CHARS: usize = 140;
...
pub async fn refresh(
    ...
    for account in &accounts {
        ...
        let provider = state
            .providers
            .provider_for_operation(&provider_account(account), ProviderOperation::Inbox, None)
            .await;
    ...
    // insert would announce items a crash could still lose. Each emit ALSO raises a
    // user notification (the Inbox shows a new-reply row with the Outpost icon), so
    // an incoming reply/mention surfaces even when nobody is watching the app.
    for item in &fresh_items {
        let body = item
            .text
            .chars()
            .take(NOTIFY_BODY_CHARS)
            .collect::<String>();
        state
            .events
            .emit_with_notify(
                ...
                Some(ryu_app_events::NotifyHint::info(
                    format!("New {} from {}", item.kind.as_str(), item.author),
...
pub async fn reply(state: &AppState, item_id: &str, text: &str) -> ApiResult<InboxItem> {
    ...
    let provider = state
        ...
        .provider_for_operation(
            &ProviderAccount {
                id: item.social_account_id.clone(),
                platform: item.platform,
                label: None,
                external_id: None,
            },
            ProviderOperation::Reply,
            None,
        )
        .await;
    match provider
        .reply_to_inbox_item(&provider_item(&item), text)
        .await
    {
        PublishResult::Ok { .. } => {}
