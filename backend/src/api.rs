...
pub fn routes(state: AppState) -> Router<()> {
    Router::new()
        ...
        .route(
            "/workspaces/:id",
            get(get_workspace).patch(patch_workspace).delete(delete_workspace),
        )
        // ── Accounts ──
...
async fn patch_workspace(
    ...
    require_hit(state.store.rename_workspace(&id, &body.name).await?, "workspace")?;
...
async fn list_posts(
    ...
    let statuses: Vec<PostStatus> = q
        ...
        .unwrap_or_default();
    let posts = state.store.list_scheduled_posts(workspace, &statuses).await?;
...
/// Compose + schedule in one call.
///
/// Accepts either an existing `draft_id` or an inline `body` (in which case a draft
/// is created first, so the content is addressable and editable afterwards). One
/// endpoint rather than "create draft, then schedule" because the composer's Publish
/// button is one user action and a half-completed pair would leave an orphan draft.
#[derive(Debug, Deserialize)]
...
#[derive(Debug, Deserialize)]
struct ValidateBody {
    /// Platform keys to check against. Unknown keys are checked against the generous
    /// default limits rather than rejected — see [`limits_for_str`].
    platforms: Vec<String>,
    #[serde(default)]
    segments: Vec<PostSegment>,
    /// Whose `enforce_platform_limits` decides whether an over-limit result BLOCKS or
    /// merely warns. Omitted (the composer before a workspace is picked) falls back to
    /// the default workspace, and a settings read that fails falls back to enforcing —
    /// the stricter side, so a storage hiccup can never silently unblock a bad post.
    #[serde(default)]
    workspace_id: Option<String>,
}
...
async fn calendar(
    ...
    let posts = state.store.list_posts_in_range(workspace, q.from, q.to).await?;
...
async fn best_times(
    ...
    let platform = match q.platform.as_deref().map(str::trim).filter(|p| !p.is_empty()) {
        Some(raw) => Some(
            Platform::parse(raw).ok_or_else(|| ApiError::bad_request(format!("unknown platform `{raw}`")))?,
        ),
        None => None,
    };
...
#[derive(Debug, Deserialize)]
struct ReadBody {
    /// Absent means "mark read". The explicit false is what lets a user un-read an
    /// item they opened by accident.
    #[serde(default)]
    read: Option<bool>,
}
...
async fn create_media(
    ...
    let mime = body.mime_type.clone().filter(|m| !m.trim().is_empty()).or_else(|| {
...
/// that echoed the Composio key back would put it in every proxy log and every
/// screenshot of this screen. Plaintext is accepted on PATCH only.
...
struct PatchSettingsBody {
    ...
    /// plaintext credentials enter the process and having them under a named key makes
    /// that boundary greppable. Its own fields are individually optional with the same
    /// "absent means unchanged" rule; an explicit `""` clears a secret.
    #[serde(default)]
...
mod tests {
    ...
    async fn validate_blocks_or_merely_warns_per_the_workspace_setting() {
        ...
        let strict = validate_post(State(state.clone()), Json(body())).await.unwrap().0;
        ...
        let warn = validate_post(State(state.clone()), Json(body())).await.unwrap().0;
