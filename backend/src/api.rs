...
//
// This is the one route whose request body accepts PLAINTEXT credentials
// (`node.composio_api_key`, `node.bluesky_app_password`, `node.threads_access_token`).
// Documenting it would lower it into an agent tool whose advertised arguments are
// "paste a Composio API key here" — an invitation for a model to invent or relay a
