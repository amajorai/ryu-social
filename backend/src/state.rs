...
/// Resolve a local media reference only beneath the operator-configured Social
/// media root. The root is app-owned by default; a desktop operator can point it
/// at a dedicated picker directory with `RYU_SOCIAL_MEDIA_ROOT`.
pub fn approved_local_media_path(raw: &str) -> anyhow::Result<PathBuf> {
    let root = std::env::var_os("RYU_SOCIAL_MEDIA_ROOT")
        .map(PathBuf::from)
        .unwrap_or_else(|| crate::paths::ryu_dir().join("social-media"));
    approved_local_media_path_in(raw, &root)
}

fn approved_local_media_path_in(raw: &str, root: &Path) -> anyhow::Result<PathBuf> {
    let path = Path::new(raw);
    if !path.is_absolute() || raw.contains('\0') {
        anyhow::bail!("media path must be an absolute local file");
    }
    std::fs::create_dir_all(&root)?;
    let root = std::fs::canonicalize(&root)
        .map_err(|e| anyhow::anyhow!("media root is unavailable: {e}"))?;
    let metadata = std::fs::symlink_metadata(path)?;
    if metadata.file_type().is_symlink() {
        anyhow::bail!("symbolic-link media is not allowed");
    }
    let canonical = std::fs::canonicalize(path)?;
    if !canonical.starts_with(&root) || !canonical.is_file() {
        anyhow::bail!("media must be beneath the configured Social media root");
    }
    Ok(canonical)
}
...
#[cfg(test)]
mod tests {
    use super::approved_local_media_path_in;

    #[test]
    fn local_media_cannot_escape_the_configured_root() {
        let base = std::env::temp_dir().join(format!("ryu-social-{}", uuid::Uuid::new_v4()));
        ...
        assert!(approved_local_media_path_in(inside.to_str().unwrap(), &root).is_ok());
        assert!(approved_local_media_path_in(outside.to_str().unwrap(), &root).is_err());
...
impl AppState {
    pub fn new(store: SocialStore, config: Config) -> Self {
        ...
        Self {
            ...
            events: ryu_app_events::EventEmitter::with_client(PLUGIN_ID, http.clone()),
        }
    }
}
