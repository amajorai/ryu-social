...
//! different "enforce platform limits" in each. Account credentials are the
//! opposite — one Bluesky app password serves the whole node, and duplicating it
//! per workspace would mean re-entering it for every workspace. Provider
//! credentials are a separate concern: Ryu's Gateway/provider vault owns them and
//! this satellite never receives or stores them.
//!
//! So the split is by OWNERSHIP, not by convenience:
//!
//! | lives in [`SocialSettings`] (per workspace) | lives in [`NodeSettings`] (per node) |
//! |---|---|
//! | `scheduler_enabled`, `poll_interval_secs` | account credentials |
