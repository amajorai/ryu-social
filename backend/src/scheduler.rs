...
pub async fn tick(state: &AppState, policy: &TickPolicy) {
    ...
    //    paying for it.
    let due = match state.store.list_due_posts(state.config.sweep_batch_size).await {
        Ok(posts) => posts,
        Err(e) => {
...
pub fn recommend_slots(samples: &[TimingSample], tz: Tz, now: i64) -> TimingRecommendation {
    ...
    let total: u64 = dated.iter().map(|s| s.engagement).fold(0, u64::saturating_add);
...
mod tests {
    ...
    async fn two_concurrent_sweeps_cannot_claim_the_same_post() {
        ...
        assert_eq!(
            s.get_scheduled_post(&post_id).await.unwrap().unwrap().status,
            PostStatus::Due
        );
    ...
    async fn boot_recovery_requeues_a_post_that_never_reached_its_first_target() {
        ...
        assert!(s.reap_expired_claims(i64::MAX, 5_000).await.unwrap().is_empty());
        assert_eq!(
            s.get_scheduled_post(&post_id).await.unwrap().unwrap().status,
            PostStatus::Publishing
        );
    ...
    async fn boot_recovery_leaves_a_post_with_a_live_target_alone() {
        ...
        assert_eq!(
            s.get_scheduled_post(&post.id).await.unwrap().unwrap().status,
            PostStatus::Publishing
        );
    }
    ...
    async fn boot_recovery_settles_a_post_whose_targets_all_finished() {
        ...
        assert!(s.settle_post(&post_id, PostStatus::Published).await.unwrap());
        assert_eq!(
            s.get_scheduled_post(&post_id).await.unwrap().unwrap().status,
    ...
    async fn a_tick_claims_due_work_and_leaves_it_recoverable_when_the_runner_declines() {
        ...
        let status = s.get_scheduled_post(&post_id).await.unwrap().unwrap().status;
        assert_ne!(status, PostStatus::Scheduled);
        assert!(!s.list_scheduled_posts(DEFAULT_WORKSPACE_ID, &[]).await.unwrap().is_empty());
    }
    ...
    fn sample(offset_days: i64, hour: i64, engagement: u64) -> TimingSample {
        TimingSample {
            published_at: MON_2026_08_10_UTC
                + offset_days * 86_400_000
                + hour * 3_600_000,
            engagement,
        }
    }
    ...
    fn next_occurrence_lands_on_the_right_local_weekday_and_hour() {
        ...
        assert_eq!(next_monday_nine, MON_2026_08_10_UTC + 7 * 86_400_000 + 9 * 3_600_000);
