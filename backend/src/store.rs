...
impl SocialStore {
    pub async fn list_workspaces(&self) -> Result<Vec<Workspace>> {
        ...
        let sql = format!("SELECT {COLS_WORKSPACE} FROM workspaces ORDER BY created_at ASC, name ASC");
    ...
    pub async fn delete_workspace(&self, id: &str) -> Result<bool> {
        ...
        for table in [
            ...
            tx.execute(&format!("DELETE FROM {table} WHERE workspace_id = ?1"), params![id])?;
        }
...
impl SocialStore {
    ...
    pub async fn get_account(&self, id: &str) -> Result<Option<SocialAccount>> {
        ...
        Ok(conn.query_row(&sql, params![id], row_to_account).optional()?)
    }
...
impl SocialStore {
    ...
    pub async fn delete_scheduled_post(&self, id: &str) -> Result<bool> {
        ...
        tx.execute("DELETE FROM post_targets WHERE scheduled_post_id = ?1", params![id])?;
...
fn attach_targets(
    ...
    for t in targets {
        by_post.entry(t.scheduled_post_id.clone()).or_default().push(t);
    }
...
impl SocialStore {
    ...
    pub async fn list_history(&self, workspace_id: &str, limit: usize) -> Result<Vec<PostHistoryEntry>> {
        let conn = self.conn.lock().await;
    ...
    pub async fn get_history(&self, id: &str) -> Result<Option<PostHistoryEntry>> {
        ...
        Ok(conn.query_row(&sql, params![id], row_to_history).optional()?)
    }
...
impl SocialStore {
    ...
    pub async fn get_template(&self, id: &str) -> Result<Option<Template>> {
        ...
        Ok(conn.query_row(&sql, params![id], row_to_template).optional()?)
    }
...
impl SocialStore {
    ...
    pub async fn upsert_media(
        ...
        let sql = format!(
            "SELECT {COLS_MEDIA} FROM media_assets WHERE workspace_id = ?1 AND path = ?2"
        );
...
impl SocialStore {
    pub async fn list_inbox(
        ...
        sql.push_str(&format!(" ORDER BY received_at DESC LIMIT ?{}", binds.len()));
        let mut stmt = conn.prepare(&sql)?;
...
impl SocialStore {
    pub async fn list_activity(&self, workspace_id: &str, limit: usize) -> Result<Vec<ActivityItem>> {
        let conn = self.conn.lock().await;
        let sql = format!(
...
impl SocialStore {
    ...
    pub async fn get_target(&self, id: &str) -> Result<Option<PostTarget>> {
        ...
        Ok(conn.query_row(&sql, params![id], row_to_target).optional()?)
    }
...
mod tests {
    ...
    async fn every_list_query_parses_against_the_real_schema() {
        ...
        assert!(s
            ...
            .unwrap()
            .is_empty());
        assert!(s.list_posts_in_range(ws, 0, i64::MAX).await.unwrap().is_empty());
    ...
    async fn the_due_claim_is_exclusive() {
        ...
        assert!(s.settle_post(&post.id, PostStatus::Published).await.unwrap());
    ...
    async fn targets_read_back_in_insertion_order_on_every_path() {
        ...
        let fetched = s.get_scheduled_post(&post.id).await.unwrap().unwrap();
        assert_eq!(
            fetched.targets.iter().map(|t| t.platform).collect::<Vec<_>>(),
            platforms.to_vec(),
    ...
    async fn the_reaper_leaves_a_post_that_still_has_a_live_target_alone() {
        ...
        let reaped = s.reap_expired_claims(5_000, 5_000).await.unwrap();
        assert_eq!(reaped, vec![b.targets[0].id.clone()], "only B's lease expired");
    ...
    async fn inbox_ingest_dedupes_and_preserves_local_state() {
        ...
        assert!(stored.read);
        assert_eq!(s.list_inbox(DEFAULT_WORKSPACE_ID, &InboxFilter::default(), 50).await.unwrap().len(), 1);
    }
    ...
    async fn media_upsert_is_idempotent_on_path() {
        let s = store().await;
        let a = s
            .upsert_media(DEFAULT_WORKSPACE_ID, "/tmp/a.png", "a.png", Some("image/png"))
            .await
            .unwrap();
        let b = s
            .upsert_media(DEFAULT_WORKSPACE_ID, "/tmp/a.png", "a.png", Some("image/png"))
    ...
    async fn workspace_delete_cascades_and_refuses_the_default() {
        ...
        let post = s
            ...
            .await
            .unwrap();
        s.insert_history(&post.targets[0].id, HistoryStatus::Published, Some("r1"), None, None)
            .await
            .unwrap();
    ...
    async fn settings_round_trip_and_default_when_unset() {
        ...
        s.put_settings(DEFAULT_WORKSPACE_ID, &settings).await.unwrap();
        assert_eq!(s.get_settings(DEFAULT_WORKSPACE_ID).await.unwrap(), settings);
    }
