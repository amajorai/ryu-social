    ...
    fn migrate(conn: &Connection) -> Result<()> {
        ...
        if current < 3 {
            conn.execute_batch(V3_DDL)
                .context("applying social schema v3")?;
        }
...
/// v3 — bind workspaces to Core's verified caller identity.
///
/// Legacy databases keep their rows unbound so a local single-user node remains
/// usable. The default workspace is claimed atomically by the first verified caller
/// that reaches it; other legacy rows are not exposed to an identified caller.
const V3_DDL: &str = "
...
impl SocialStore {
    ...
    /// Return only workspaces visible to a verified caller. `None`/`None` is the
    /// local single-user node authority; identified callers match their personal
    /// workspace or their verified node organization.
    pub async fn list_workspaces_for_caller(
        &self,
        user_id: Option<&str>,
        org_id: Option<&str>,
        ...
        let conn = self.conn.lock().await;
        let sql = format!(
            "SELECT {COLS_WORKSPACE} FROM workspaces
             WHERE owner_user_id = ?1 OR (org_id IS NOT NULL AND org_id = ?2)
             ORDER BY created_at ASC, name ASC"
        );
        let mut stmt = conn.prepare(&sql)?;
        let rows = stmt.query_map(
            params![user_id.unwrap_or(""), org_id.unwrap_or("")],
            row_to_workspace,
        )?;
        Ok(rows.collect::<rusqlite::Result<Vec<_>>>()?)
    }
    ...
    /// Check the workspace boundary before a route reads or mutates a child row.
    /// The legacy default is claimed once, atomically, by the first verified user;
    /// all other unbound legacy workspaces remain local-only.
    pub async fn workspace_accessible(
        &self,
        id: &str,
        user_id: Option<&str>,
        org_id: Option<&str>,
        ...
        let conn = self.conn.lock().await;
        let owners: Option<(Option<String>, Option<String>)> = conn
            .query_row(
                "SELECT owner_user_id, org_id FROM workspaces WHERE id = ?1",
                params![id],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
            .optional()?;
        let Some((owner, workspace_org)) = owners else {
            return Ok(false);
        };
        if user_id.is_none() && org_id.is_none() {
            return Ok(true);
        }
        if user_id.is_some_and(|user| owner.as_deref() == Some(user))
            || org_id.is_some_and(|org| workspace_org.as_deref() == Some(org))
        {
            return Ok(true);
        }
        if id == DEFAULT_WORKSPACE_ID && owner.is_none() && workspace_org.is_none() {
            let Some(user_id) = user_id else {
                return Ok(false);
            };
            let changed = conn.execute(
                "UPDATE workspaces
                    SET owner_user_id = ?2, org_id = ?3
                  WHERE id = ?1 AND owner_user_id IS NULL AND org_id IS NULL",
    ...
    pub async fn create_workspace(&self, name: &str) -> Result<Workspace> {
        self.create_workspace_for_caller(name, None, None).await
    }

    pub async fn create_workspace_for_caller(
        &self,
        name: &str,
        ...
        conn.execute(
            "INSERT INTO workspaces
               (id, name, created_at, owner_user_id, org_id)
             VALUES (?1, ?2, ?3, ?4, ?5)",
            params![ws.id, ws.name, ws.created_at, owner_user_id, org_id],
        )?;
        Ok(ws)
    }
...
impl SocialStore {
    ...
    pub async fn workspace_for_account(&self, id: &str) -> Result<Option<String>> {
        let conn = self.conn.lock().await;
        Ok(conn
            .query_row(
                "SELECT workspace_id FROM social_accounts WHERE id = ?1",
                params![id],
                |row| row.get(0),
            )
            .optional()?)
...
impl SocialStore {
    ...
    pub async fn workspace_for_post(&self, id: &str) -> Result<Option<String>> {
        let conn = self.conn.lock().await;
        Ok(conn
            .query_row(
                "SELECT workspace_id FROM scheduled_posts WHERE id = ?1",
                params![id],
                |row| row.get(0),
            )
            .optional()?)
...
impl SocialStore {
    ...
    pub async fn workspace_for_history(&self, id: &str) -> Result<Option<String>> {
        let conn = self.conn.lock().await;
        Ok(conn
            .query_row(
    ...
    pub async fn workspace_for_draft(&self, id: &str) -> Result<Option<String>> {
        let conn = self.conn.lock().await;
        Ok(conn
            .query_row(
                "SELECT workspace_id FROM drafts WHERE id = ?1",
                params![id],
                |row| row.get(0),
            )
            .optional()?)
...
impl SocialStore {
    ...
    pub async fn workspace_for_template(&self, id: &str) -> Result<Option<String>> {
        let conn = self.conn.lock().await;
        Ok(conn
            .query_row(
                "SELECT workspace_id FROM templates WHERE id = ?1",
                params![id],
                |row| row.get(0),
            )
            .optional()?)
...
impl SocialStore {
    ...
    pub async fn workspace_for_media(&self, id: &str) -> Result<Option<String>> {
        let conn = self.conn.lock().await;
        Ok(conn
            .query_row(
                "SELECT workspace_id FROM media_assets WHERE id = ?1",
                params![id],
                |row| row.get(0),
            )
            .optional()?)
...
impl SocialStore {
    ...
    pub async fn workspace_for_inbox(&self, id: &str) -> Result<Option<String>> {
        let conn = self.conn.lock().await;
        Ok(conn
            .query_row(
                "SELECT workspace_id FROM inbox_items WHERE id = ?1",
                params![id],
                |row| row.get(0),
            )
            .optional()?)
...
mod tests {
    ...
    #[tokio::test]
    async fn workspaces_are_scoped_to_verified_users_and_orgs() {
        let s = store().await;
        let alice = s
            .create_workspace_for_caller("Alice", Some("user-a"), Some("org-a"))
            .await
            .unwrap();
        let bob = s
            .create_workspace_for_caller("Bob", Some("user-b"), Some("org-b"))
            .await
            .unwrap();
        assert!(s
            .workspace_accessible(&alice.id, Some("user-a"), Some("org-a"))
            .await
            .unwrap());
        assert!(!s
            .workspace_accessible(&alice.id, Some("user-b"), Some("org-b"))
            .await
            .unwrap());
        assert!(s
            .workspace_accessible(&alice.id, Some("user-c"), Some("org-a"))
            .await
            .unwrap());
        assert_eq!(
            s.list_workspaces_for_caller(Some("user-b"), Some("org-b"))
                .await
                .unwrap()
                .into_iter()
