...
async fn main() -> anyhow::Result<()> {
    ...
    if token.is_some() {
        tracing::info!(
            "ryu-social: protected {MOUNT}/* routes require the injected shared-secret bearer"
        );
    } else {
        tracing::warn!(
    ...
    // Re-resolve the provider registry against persisted account settings before
    // anything can serve a request. Provider credentials belong to Ryu's managed
    // Gateway and never live in this app's settings blob.
    //
    // `AppState::new` stays synchronous (every test builds a state through it), so
    // the registry it installs can only resolve the managed Gateway bridge and
    // environment account settings. This async pass only supplies Bluesky account
    // auth; provider keys are never read from the node settings blob.
    //
    ...
    match settings::load(&store).await {
        Ok(node) => {
            // Normalize the node blob on boot so installations that briefly had
            // the old app-owned `treg_token` field do not retain that credential
            // after upgrading to Ryu-managed provider routing.
            if let Err(error) = settings::save(&store, &node).await {
                tracing::warn!(error = %error, "ryu-social: could not normalize node settings");
            }
            state.providers = providers::ProviderRegistry::from_credentials_with_router(
                state.http.clone(),
                providers::ProviderCredentials::from_node_settings(&node),
                ryu_app_events::ProviderRouter::with_client(
    ...
    //
    // `/openapi.json` rides INSIDE that same gate, but at the SERVER ROOT rather than
    // under the mount: Core fetches `http://127.0.0.1:<port>/openapi.json` first on
    // this sidecar's Healthy edge and lowers each documented operation into an agent
    // tool. Root, because `http.mount` says where the ROUTES nest, not where the
    // server is rooted — and the document's own paths already carry the mount. Inside
    // the gate, because the schema enumerates this app's whole surface and is not
    // something an unauthenticated caller on loopback should be able to read; Core
    // stamps the same bearer on this fetch as on every proxied hop.
