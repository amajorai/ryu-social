...
use crate::providers::{
    PlatformProvider, ProviderAccount, ProviderOperation, PublishMedia, PublishRequest,
    PublishResult, PublishSegment,
};
use crate::state::{AppState, EVENT_POST_FAILED, EVENT_POST_PUBLISHED, PROVIDER_CALL_TIMEOUT_MS};
...
async fn settle_post(
    ...
    let status = aggregate_target_statuses(&persisted.iter().map(|t| t.status).collect::<Vec<_>>());
    ...
    // a live Core and a down Core never blocks a publish. A completed publish raises
    // a user notification (the Inbox row shows the Outpost icon) so the outcome
    // surfaces even when nobody is watching the app.
    match status {
        PostStatus::Published => {
            state
                .events
                .emit_with_notify(
                    ...
                    payload,
                    Some(
                        ryu_app_events::NotifyHint::info("Post published", None)
                            .with_level("success"),
                    ),
                )
                .await
        ...
        PostStatus::Partial | PostStatus::Failed => {
            state
                ...
                .emit_with_notify(
                    ...
                    payload,
                    Some(
                        ryu_app_events::NotifyHint::info("Post failed to publish", None)
                            .with_level("error"),
                    ),
                )
                .await;
...
async fn run_target(
    ...
    for segment in &content.segments {
        for media in &segment.media {
            let lower_path = media.path.to_ascii_lowercase();
            if lower_path.starts_with("http://") || lower_path.starts_with("https://") {
                continue;
            }
            if let Err(error) = crate::state::approved_local_media_path(&media.path) {
                return fail_locally(
                    state,
                    target,
                    format!("local media is not allowed: {error}"),
                )
                .await;
            }
        }
    ...
    let provider = state
        .providers
        .provider_for_operation(&content.account, ProviderOperation::Publish, Some(&request))
        .await;
    attempt_publish(
        state,
        target,
...
async fn publish_once(provider: &dyn PlatformProvider, request: &PublishRequest) -> PublishResult {
...
mod tests {
    ...
    async fn due_post(state: &AppState, body: &DraftBody, platforms: &[Platform]) -> ScheduledPost {
        ...
        let post = state
            .store
            .create_scheduled_post(DEFAULT_WORKSPACE_ID, Some(&draft.id), now_ms(), &targets)
            .await
            .unwrap();
        assert!(state
            .store
    ...
    async fn every_attempt_of_a_run_reuses_the_same_idempotency_key() {
        ...
        let keys: Vec<Option<String>> = calls.iter().map(|c| c.idempotency_key.clone()).collect();
        ...
        let expected = idempotency_key_for(&post.id, &post.targets[0].social_account_id);
