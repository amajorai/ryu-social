...
use crate::providers::{ProviderAccount, ProviderOperation, RemotePostRef};
...
pub fn rollups(items: &[ActivityItem], utc_offset_minutes: i32) -> AnalyticsRollups {
    ...
    for item in items {
        ...
        let day_start_ms = local
            ...
            .and_hms_opt(0, 0, 0)
            .map(|midnight| {
                midnight.and_utc().timestamp_millis() - i64::from(utc_offset_minutes) * 60_000
            })
    ...
    per_platform
        .sort_by(|(pa, a), (pb, b)| b.engagement.cmp(&a.engagement).then_with(|| pa.cmp(pb)));
